...
$result = $conn->query("INSERT INTO $table VALUES($pid,'$pname','$pdesc','$pcategory',$pprice,'$pimage')");
 ...
 function AddSupplier($conn,$username,$db,$password,$firstname,$email,$address,$dob,$gender,$SupplyItem,$interests,$uimage)
 {
...
$result = $conn->query("SELECT * FROM  $table");
 ...
 function ShowAllSupplier($conn,$table){
    $result = $conn->query("SELECT * FROM ".$table);
    return $result;
 }

 function showOneSupplier($conn,$table,$username){
